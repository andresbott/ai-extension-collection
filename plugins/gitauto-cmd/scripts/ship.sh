...
#                                 wait-ci → merge → sync-main → cleanup → tag
#
# prepare prints `DONE ...` (nothing for the model to do) or a `SHIP ...` context
# block. run prints exactly one final line: SHIPPED, PARTIAL, or STOPPED; the full
...
prepare() {
  local hint="${1:-}" current base ref mb protected=no pr=none dirty ahead files lines dirs need writer=cheap
  ...
  case "$pr" in
    merged#*)
      ...
      need=none ;;
    open#*) if [[ "$dirty" -gt 0 ]]; then need=message; else need=none; fi ;;
  ...
  [[ "$need" == none || "$writer" == expert ]] && return 0
...
draft() {
  ...
  printf '%s\n' "$body" > "$draft_dir/body"
  printf 'DRAFTED title=%s\n' "$title"
}

run() {
  local branches=() title='' message='' body='' tag='' delete_remote=false
  while [[ $# -gt 0 ]]; do
    case "$1" in
  ...
  local log merged=no out st branch number='' url='' subject worktree='' summary n=0 mark=1
  ...
  if [[ "$st" == already-merged ]]; then
    ...
    subject="$title"
    [[ -n "$subject" ]] || subject="$(gh pr view "$number" --json title --jq .title 2>/dev/null || true)"
  ...
  if [[ -n "$tag" ]]; then
    step tag "$D/gitauto-tag.sh" "version=$tag" "subject=${title:-}" remote=origin "worktree=${worktree:-.}"
    summary+=" tag=$st"
...
case "$cmd" in
  ...
  draft) draft "$@" ;;
  run) run "$@" ;;
  *) fail "usage: ship.sh prepare|draft|run" ;;
esac
