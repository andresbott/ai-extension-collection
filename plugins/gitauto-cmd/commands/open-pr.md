---
description: Open (or reuse) a PR for the current work and wait until CI is green — branch, verify, commit, push, open PR, watch CI. Does not merge; run /gitauto-cmd:ship afterwards to land it.
...
- `need=pr writer=cheap`: from the context above, write:
  ...
  - a **subject**: one Conventional Commit line, `type(scope): summary` (types:
    ...
    It is saved in the PR and becomes the squash subject when `/gitauto-cmd:ship` merges;
...
- its last line verbatim (`READY`, `STOPPED`, or a final `WAITING`); after
  `READY`, add "run /gitauto-cmd:ship to merge";
