...
## Detection methodology (import-graph first)

Work the package import graph first; open source files only to quote a finding
you have already localized.
...
1. **Map the tiers.** Classify each package in the graph into a tier per the
   model above (grounded in the project's own docs).

2. **Find cross-tier / sideways edges.** Scan the graph for edges where one
   delivery package imports another, or where a domain/infra package imports the
   delivery tier. Note import cycles and packages with unusually high fan-in or
   fan-out.
